...
<?php if (isset($_POST['delete'])) {
    $delete = $_POST['delete'];
    $SQL = $odb -> prepare("SELECT * FROM `logs_free` WHERE `id` = ?");
    $SQL -> execute(array($delete));
    $verif = $SQL->fetch();
    if ($verif['username'] == $_SESSION['username']) {
        $SQL = $odb -> prepare("UPDATE `logs_free` SET `visible` = ? WHERE `id` = ?");
...
if (isset($_POST['favori'])) {
    ...
    $verif = $SQL->fetch();
    if ($verif['username'] == $_SESSION['username']) {
        $SQL = $odb -> prepare("UPDATE `logs_free` SET `favori` = ? WHERE `id` = ?");
...
<div class="bg-body-light">
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">History of my free dispensers</h1>
...
<div class="content">
    ...
    <?php
    $SQLSelect1 = $odb->query("SELECT * FROM `logs_free` WHERE `username`='{$_SESSION['username']}' AND visible = 0 AND favori = 1 ORDER BY date DESC");
    ...
    if ($test < 1) { }
        ...
        <div class="block block-rounded block-bordered">
            <div class="block-header block-header-default">
                <h3 class="block-title">Table of your favorites accounts</h3>
            </div>
    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            <h3 class="block-title">Dynamic list of all your Dispensering</h3>
            <div class="block-options">
                <form action="inc/form/form.php" method="post">
                    <button type="submit" name="vider_histo_free" class="btn btn-sm btn-outline-danger">Delete my history</button>
                </form>
            </div>
        </div>
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-buttons">
                ...
                <?php
                $SQLSelect = $odb->prepare('SELECT * FROM `logs_free` LEFT JOIN report_log ON logs_free.id = report_log.id_log WHERE `username`= ? AND visible = 0 AND favori = 0 ORDER BY date DESC');
